...
function TypeFilter() {
  ...
  // Sync selectedTypes with URL when it changes
  useEffect(() => {
    const typesFromUrl = searchParams.get("type")?.split(",") || [];
    setSelectedTypes(typesFromUrl);

    // Close dropdown when type filter is cleared
  ...
  const typeHandler = (e) => {
    ...
    const queryString = createQueryString("type", newTypes);
      ...
      <div className="p-4 cursor-pointer select-none" onClick={toggleDropdown}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <LuNewspaper className="w-5 h-5 text-secondary-500" />
            <span className="font-medium text-secondary-900">نوع محصولات</span>
          </div>
          <motion.div
